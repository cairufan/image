...
- [EMMC升级常见问题及流程梳理](#emmc升级常见问题及流程梳理)
  ...
  - [5. 常见问题整理](#5-常见问题整理)
    ...
    - [5.7 如何强制进入备份系统进行恢复升级](#57-如何强制进入备份系统进行恢复升级)
...
### 5.7 如何强制进入备份系统进行恢复升级

```shell

前提条件是有板子内烧录有正常能用的备份升级的分区（一般都有），加密芯片号和升级包的板子上的加密芯片串号匹配（一般都匹配）；
准备一个U盘 在U盘的根目录下创建一个upgrade目录，把升级包放到此目录下，U盘插入设备

前面的准备工作完成后，重启设备，按住ctrl+c进入到UBOOT下的CMD命令界面，输入以下命令:

status clr crc;status set cnt 3;reset

会强制进入到备份系统进行恢复升级，等待升级完成即可；（若无法升级 保留完整打印给BSP分析）
